...
- 要求：
1. 实现 ls(按字符排列) 的 -a(显示隐藏文件排列)、-l(详细排列)、-R(递归排列)、-t(按照最新一次修改时间降序排列)、-r(逆序排列)、 -i(显示inode编号排列)、-s(显示已用内存块数量排列) 参数，并允许这些参数任意组合。
...
## scandir(dirpath,dp,fitterFun,compareFun)
- 定义：一次性将所有目标展示文件与目录导入dp结构体中
